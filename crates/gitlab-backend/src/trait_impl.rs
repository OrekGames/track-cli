...
impl IssueTracker for GitLabClient {
    ...
    fn search_issues(&self, query: &str, limit: usize, skip: usize) -> Result<SearchResult<Issue>> {
        let (search_text, state, labels, assignee) = convert_query_to_gitlab_params(query);
        let project_id = self.project_id_str();
        ...
        while issues.len() < limit {
            ...
            let (page_issues, page_total) = if search_text.is_empty() {
                self.list_issues_with_total(state.as_deref(), per_page, page, assignee.as_deref())?
            } else {
    ...
    fn get_issue_count(&self, query: &str) -> Result<Option<u64>> {
        let (search_text, state, _labels, assignee) = convert_query_to_gitlab_params(query);
        Ok(self.count_issues_by_query(&search_text, state.as_deref(), assignee.as_deref())?)
    }
