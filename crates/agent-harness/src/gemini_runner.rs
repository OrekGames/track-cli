//! Gemini CLI runner
//!
//! Invokes Gemini CLI as a subprocess and captures its behavior for evaluation.
//! This allows testing how Gemini performs on scenarios using the track CLI.
//! Uses Gemini's stream-json output format for structured event parsing.
...
/// Output event from Gemini CLI stream-json format
...
pub fn run_gemini(config: &GeminiRunnerConfig) -> Result<GeminiRunnerResult> {
    ...
    if config.verbose {
        println!("\n{}", "Starting Gemini CLI session...".cyan());
    ...
    // Combine system and task prompts for Gemini
    ...
    // Spawn process
    let mut child = cmd.spawn().context("Failed to spawn Gemini CLI")?;
    ...
    // Wait for completion
    let status = child.wait().context("Failed to wait for Gemini CLI")?;
...
fn parse_stream_output(stdout: impl std::io::Read, verbose: bool) -> Result<Vec<GeminiEvent>> {
...
fn extract_commands(events: &[GeminiEvent]) -> Vec<CommandExecution> {
...
fn count_turns(events: &[GeminiEvent]) -> usize {
    events
        .iter()
        .filter(|e| matches!(e, GeminiEvent::Assistant(_)))
        .count()
}

/// Extract final result text
fn extract_final_result(events: &[GeminiEvent]) -> Option<String> {
    events.iter().rev().find_map(|e| {
        if let GeminiEvent::Result(res) = e {
            res.result.clone()
        } else {
            None
        }
...
/// Convert GeminiRunnerResult to SessionResult for evaluation
...
mod tests {
    ...
    fn test_parse_assistant_event() {
        ...
        if let GeminiEvent::Assistant(asst) = event {
