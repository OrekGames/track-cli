...
pub fn jira_issue_to_core(j: JiraIssue, jira_fields: &[JiraField]) -> Issue {
    ...
    let has_timetracking = j.fields.extra.contains_key("timetracking");
    ...
    for (key, value) in &j.fields.extra {
        ...
        if has_timetracking && (key == "timeoriginalestimate" || key == "timeestimate") {
            continue;
        }
        if value.is_null() {
            continue;
...
mod tests {
    ...
    #[test]
    fn jira_issue_to_core_skips_timetracking_duplicates() {
        let mut extra = std::collections::HashMap::new();
        // Jira provides both aliases and the composite object at the top level
        ...
        let orig = core.custom_fields.iter().filter(|f| matches!(f, CustomField::Text { name, .. } if name == "Original Estimate")).collect::<Vec<_>>();
        let rem = core.custom_fields.iter().filter(|f| matches!(f, CustomField::Text { name, .. } if name == "Remaining Estimate")).collect::<Vec<_>>();

        // Should not duplicate the fields
        assert_eq!(orig.len(), 1);
        assert_eq!(rem.len(), 1);

        // Should prefer the composite format (e.g. "4h" over 14400)
        assert!(matches!(orig[0], CustomField::Text { value: Some(v), .. } if v == "4h"));
        assert!(matches!(rem[0], CustomField::Text { value: Some(v), .. } if v == "2h"));
    }
