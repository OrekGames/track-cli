...
impl JiraClient {
    ...
    pub fn resolve_transition_id(&self, issue_key: &str, target_status: &str) -> Result<String> {
        ...
        if let Some(t) = transitions
            .iter()
            // Bolt: Replace O(N) string allocations (.to_lowercase()) with allocation-free unicode_eq_ignore_case
            .find(|t| tracker_core::unicode_eq_ignore_case(&t.to.name, target))
        {
